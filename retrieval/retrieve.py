...
def get_data(wfc3=True, stis=True, spitz=True, eclipse=False):
    ...
    if eclipse == True:
        ...
        nan = float('NaN')
        if stis:
            # Can check nik's new numbers here
            #stis_dict = readsav('HAT41_STIS.sav')
            #stis_dict = readsav('HAT41_555.sav')
            stis_dict = readsav('new_STIS.sav')
            ...
            depth_red = stis_dict['rprs_g750']**2 #+ 0.000065# DONT FORGET THIS OFFSET 
            ...
            depthsb =  depth_stis[29:46].copy()
            errorsb = error_stis[29:46].copy()
            stis_dict = readsav('HAT41_visit86_G750L.sav')
            waves2=stis_dict['w_cen']/1e10
            spread2 = stis_dict['we']/1e10
            depths2 = stis_dict['transit_rprs']**2
            errors2 = stis_dict['transit_rprs_err']*stis_dict['transit_rprs']*2.0
            var1 = 1/errorsb/errorsb
            var2 = 1/errors2/errors2
            c1 = var1*depthsb
            c2 = var2*depths2
            vartot = var1 + var2
            ctot = c1 + c2
            depths3 = ctot/vartot
            errors3= np.sqrt(1/vartot)
            depth_stis[29:46]=depths3
            error_stis[29:46]=errors3
        ...
        if spitz:
            ...
            # Hannah's spitzer points, I think? Note made 7/13/20
        ...
        if wfc3:
            # can change input here
            wfc = pd.read_csv('../bin_analysis/spectra.csv',
                              index_col=[0, 1, 2]).sort_index()
            wfc = wfc.loc[('hatp41/visit01/reverse', 'marg', 4)]
            ##############
            # Tsiaras spectrum 
            #wfc2=pd.read_csv('../bin_analysis/tsarias2.csv')
            #wfc['Central Wavelength']=wfc['wave']
            #wfc['Wavelength Range'] = (1.1372-1.1153)/2.
            #wfc['Depth'] = wfc['depth']
            #wfc['Error'] = wfc['error']
            ###########
...
if __name__ == '__main__':
    ...
    # Code to save spectrum as a csv file and as a pickle file, separated by instrument
    #spec = pd.DataFrame()
    #spec['Wave center'] = (bins[:,0]+bins[:,1])/2.
    #spread=bins.mean(axis=1)-bins.T[0]
    #spec['Bin Width'] = spread
    #spec['Depth'] = depths
    #spec['Depth Error'] = errors
    #labels = ['G430L']*29+['G750L']*17+['WFC3']*29+['Spitzer']*2
    #spec['Instrument'] = labels
    #print spec
    #spec.to_csv('../../july_spectrum.csv')
    #sys.exit()
    #h41 = {}
    #h41['g430_depths']=depths[:29] 
    #h41['g750_depths']=depths[29:46] 
    #h41['wfc3_depths']=depths[46:75]  
    #h41['spitzer_depths']=depths[75:] 
    #h41['g430_errors']=errors[:29] 
    #h41['g750_errors']=errors[29:46] 
    #h41['wfc3_errors']=errors[46:75]  
    #h41['spitzer_errors']=errors[75:] 
    #h41['g430_bins']=bins[:29] 
    #h41['g750_bins']=bins[29:46] 
    #h41['wfc3_bins']=bins[46:75]  
    #h41['spitzer_bins']=bins[75:]
    #print h41
    #with open('HATP41_spectrum.pkl', 'w') as f:
    #    pickle.dump(h41, f)
    #sys.exit()
    #spread=bins.mean(axis=1)-bins.T[0]
    #plt.errorbar(bins.mean(axis=1), depths, errors, spread, color='r', marker='o', ecolor='r', ls='',markersize=4, alpha=.5, label='paper')
    #plt.show()
    #wavesb = bins.mean(axis=1)[29:46]
    """depthsb =  depths[29:46].copy()
    errorsb = errors[29:46].copy()
    #spreadb =  spread[29:46]
    #plt.errorbar(bins.mean(axis=1)[29:46], depths[29:46], errors[29:46], spread[29:46], color='r', marker='o', ecolor='r', ls='',markersize=4, alpha=.5)
    #plt.errorbar(waves, depths, errors, spread, color='r', marker='o', ecolor='r', ls='',markersize=4, alpha=.5, label='visit85')
    ...
    depths2 = stis_dict['transit_rprs']**2
    errors2 = stis_dict['transit_rprs_err']*stis_dict['transit_rprs']*2.0
    #plt.errorbar(waves2, depths2, errors2, spread2, color='b', marker='o', ecolor='b', ls='',markersize=4, alpha=.5, label='visit86')
    print depths
    var1 = 1/errorsb/errorsb
    var2 = 1/errors2/errors2
    c1 = var1*depthsb
    ...
    depths3 = ctot/vartot
    errors3= np.sqrt(1/vartot)
    depths[29:46]=depths3
    errors[29:46]=errors3
    #plt.errorbar(bins.mean(axis=1), depths, errors, spread, color='g', marker='o', ecolor='g', ls='',markersize=4, alpha=.5, label='Weighted')
    #plt.xscale('log')
    #plt.xlim([.3e-6,1.e-6])
    #plt.show()
    #print depths
    #sys.exit()
    #plt.plot(waves, np.zeros_like(waves))"""

    #plt.legend()
    #plt.show()
    #sys.exit()
    #spread=bins.mean(axis=1)[29:46]-bins[29:46].T[0]
    #spread2=bins[29:46].T[1]-bins.mean(axis=1)[29:46]
    #plt.errorbar(bins.mean(axis=1)[29:46], depths[29:46], errors[29:46],
    #             spread, color='r', marker='o', ecolor='r', ls='',markersize=4, alpha=.5, label='5.55')
    #print bins
    #plt.legend()
    #print np.median(depths[:29])*1e6
    #print np.median(depths[29:46])*1e6
    #avgd=depths[23:28].mean()
    #stis_dict = readsav('HAT41_STIS_first_red_bin.sav')
    #bwave = stis_dict['w_g430']
    #berr = stis_dict['we_g430']
    #rwave= stis_dict['w_g750']
    #rerr = stis_dict['we_g750']
    #depth_blue = stis_dict['rprs_g430']**2
    #error_blue = stis_dict['rprs_err_g430'] * 2 * np.sqrt(depth_blue)
    #depth_red = stis_dict['rprs_g750']**2
    #error_red = stis_dict['rprs_err_g750'] * 2 * np.sqrt(depth_red)
    #plt.errorbar(bwave/1e10, depth_blue, error_blue, berr/1e10, color='purple', marker='x', ecolor='purple', ls='')
    #plt.errorbar(rwave/1e10, depth_red, error_red, rerr/1e10, color='orange', marker='x', ecolor='orange', ls='')
    #plt.errorbar(rwave/1e10, avgd, errors[27], color='g', marker='^', ecolor='g', ls='')
    #plt.show()
    #sys.exit()
    #plt.errorbar(bins.mean(axis=1)[46:75], depths[46:75], errors[46:75], color='r', marker='o', ecolor='r', ls='',markersize=4)
    ...
    if eclipse == True:
        ...
        tp.set_parametric(1590, 250, .25, .65, 1.25e5, 2550)

        calc = EclipseDepthCalculator()
        wave, depths = calc.compute_depths(tp, Rs, Mp, Rp_guess, Ts)
        plt.plot(wave, depths)
        plt.show()
        ...
        fit_info.add_uniform_fit_param('Rp', 0.9*Rp_guess, 1.1*Rp_guess)
        # fit_info.add_uniform_fit_param('T', 0.5*T_guess, 1.5*T_guess)
        fit_info.add_uniform_fit_param('T0', 0.5*T0, 3000)
        fit_info.add_uniform_fit_param('T3', 0.5*T3, 3000)
        fit_info.add_uniform_fit_param('log_P1', 1, 4)
        fit_info.add_uniform_fit_param('log_P3', 5, 7)
        fit_info.add_uniform_fit_param('alpha1', .1, 4)
        ...
        fit_info.add_uniform_fit_param("CO_ratio", .2, 2)
    ...
    if ri is not None:
        ...
        fit_info = retriever.get_default_fit_info(Rs=Rs, Mp=Mp, Rp=Rp_guess,
                                                  ...
                                                  log_part_size=-6.5,
                                                  frac_scale_height=0.0,
                                                  T_spot=Ts+100,
                                                  spot_cov_frac=0.0) 
        # Mie theory stuff
        fit_info.add_uniform_fit_param('log_number_density', 1, 20) # 1-20
        ...
        fit_info = retriever.get_default_fit_info(Rs=Rs, Mp=Mp, Rp=Rp_guess,
                                                  ...
                                                  log_cloudtop_P=0,
                                                  T_star=Ts,
                                                  cloud_fraction=1., wfc3_offset=0,
                                                  stis1_offset=0, stis2_offset=0,
                                                  stis_offset=0, spitzer_offset=0,
                                                  T_spot=Ts+100,
                                                  spot_cov_frac=0.0,
    ...
    fit_info.add_gaussian_fit_param('Rs', 0.08*R_sun)
    fit_info.add_gaussian_fit_param('Mp', 0.14*M_jup)
    ...
    #fit_info.add_uniform_fit_param("logZ", -1, 3)
    fit_info.add_uniform_fit_param("logZ", -1, 3.0)
    fit_info.add_uniform_fit_param("CO_ratio", .2, 2)
    ...
    # fit_info.add_uniform_fit_param("spot_cov_frac",0.0,0.10)
    # fit_info.add_uniform_fit_param("T_spot", 6150, 6900)
    # whitelight error from marg is 5 (50 ppm), and gaussian

    #fit_info.add_uniform_fit_param("wfc3_offset", -50, 50)
    #fit_info.add_uniform_fit_param("stis1_offset", -50, 50)
    #fit_info.add_uniform_fit_param("stis2_offset", -50, 50)
    #fit_info.add_uniform_fit_param("stis_offset", -50, 50)
    # fit_info.add_uniform_fit_param("spitzer_offset", -50, 50)
    
    #fit_info.add_gaussian_fit_param("wfc3_offset", 8.)
    #fit_info.add_gaussian_fit_param("stis1_offset", 10.5)
    #fit_info.add_gaussian_fit_param("stis2_offset", 8.5)
    
    # fit_info.add_gaussian_fit_param("wfc3_offset", 20.)
    # fit_info.add_gaussian_fit_param("stis1_offset", 20.)
