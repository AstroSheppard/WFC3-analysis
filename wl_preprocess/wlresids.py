...
def wlresids(visit, transit=False):
   ...
   # specerr=np.sqrt(np.ma.sum(allerr*allerr, axis=1))
    ...
    for s, par in enumerate(params):
        ...
        model=wl.lightcurve(par, date, sh, HSTphase, transit=transit)
        resids=(fluxnorm-model)
        #plt.errorbar(date, fluxnorm, errnorm, marker='o', ls='', label='Data')
        #plt.plot(date, model, marker='o', ls='', label='Model')
        #plt.show()
        ...
        #plt.plot(date[norm1:], 1-sys_residuals[s,norm1:], 'bo', label='array')
