...
def wave_solution(visit, dire, plotting=False, savename=False, phase=False, transit=False):
    ...
    spec=np.mean(fits.open(data[0])[0].data, axis=0) # make sure axis is right
    err=np.sqrt(spec)
    # Normalize
    err=err/np.max(spec)
    ...
    if plotting == True:
        p=plt.errorbar(data_wave/1e4, spec, err)
    ...
    if savename:
        ...
        try:
            ...
            current=current.drop(visit+'/'+dire)
...
if __name__ == '__main__':
    ...
    wave=wave_solution(visit, direction, plotting=plotting, savename=True, transit=trans)
